...
export class EntryClient extends BaseClient {
     ...
     * @return Ok
     */
    updateEntry(harvestId: string, updatedEntryParams: NewEntryParams, entryIndex: number): Observable<HarvestVm> {
        ...
        if (entryIndex === undefined || entryIndex === null)
            throw new Error("The parameter 'entryIndex' must be defined and cannot be null.");
        else
            url_ += "entryIndex=" + encodeURIComponent("" + entryIndex) + "&"; 
        url_ = url_.replace(/[?&]$/, "");
...
export class HarvestClient extends BaseClient {
     ...
     * @return Ok
     */
    getHarvestsByDateRange(dateRange: Date[]): Observable<HarvestVm[]> {
        let url_ = this.baseUrl + "/harvests/date";
        url_ = url_.replace(/[?&]$/, "");

        const content_ = JSON.stringify(dateRange);
        ...
        return Observable.fromPromise(this.transformOptions(options_)).flatMap(transformedOptions_ => {
            ...
            return this.processGetHarvestsByDateRange(response_);
        }).catch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetHarvestsByDateRange(<any>response_);
                } catch (e) {
                    return <Observable<HarvestVm[]>><any>Observable.throw(e);
                }
            } else
    ...
    protected processGetHarvestsByDateRange(response: HttpResponseBase): Observable<HarvestVm[]> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
...
export class SystemClient extends BaseClient {
     ...
     * @return Ok
     */
    uploadFile(csv: FileParameter): Observable<any> {
        let url_ = this.baseUrl + "/system/uploadFile";
        url_ = url_.replace(/[?&]$/, "");

        const content_ = new FormData();
        if (csv === null || csv === undefined)
            throw new Error("The parameter 'csv' cannot be null.");
        else
            content_.append("csv", csv.data, csv.fileName ? csv.fileName : "csv");
...
export class EntryVm implements IEntryVm {
    ...
    constructor(data?: IEntryVm) {
        ...
        if (!data) {
            ...
            this.harvester = new HarvesterVm();
            this.recipient = new OrganizationVm();
        }
    }
        ...
        if (data) {
            ...
            if (data["dateRange"] && data["dateRange"].constructor === Array) {
                ...
                for (let item of data["dateRange"])
                    this.dateRange.push(new Date(item));
            }
        }
    }
